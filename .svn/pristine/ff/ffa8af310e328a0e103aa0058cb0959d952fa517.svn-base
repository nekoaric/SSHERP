...
Ext.onReady(function () {
	...
	var store = new Ext.data.Store({
		...
		proxy: new Ext.data.HttpProxy({
			url: './ordDayList.ered?reqCode=queryOrdDayList'
		}),
	...
	var tbar = new Ext.Toolbar({
		items: ['订单号 ',{
			...
			'数量性质', nature_combo,'日期 ',{
				...
				iconCls: 'page_excelIcon',
				handler: function () {
					exportExcel(encodeURI('.//excelExport.ered?reqCode=ordDayListExcel&ord_seq_no='+Ext.getCmp('ord_seq_no').getValue()+
                                '&natures='+nature_combo.getValue()+
                                '&tr_date='+Ext.util.Format.date(Ext.getCmp("tr_date").getValue(),'Y-m-d')));
				}
			}]
	});
