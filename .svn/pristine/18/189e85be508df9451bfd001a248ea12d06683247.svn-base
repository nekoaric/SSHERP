...
Ext.onReady(function() {
	...
	var areaTree = new Ext.tree.TreePanel( {
		loader : new Ext.tree.TreeLoader( {
			baseAttrs : {},
			dataUrl : './sysGrps.ered?reqCode=areaTreeInit'
		}),
		root : root,
		title : '',
		applyTo : 'areaTreeDiv',
	...
	areaTree.on('click', function(node) { // 左键单击
				//deptid = node.attributes.id;
			treeareaid = node.attributes.id;
			store.load( {
				params : {
					start : 0,
					limit : bbar.pageSize,
					usertype : '8',
					grp_id_pattern : treeareaid
				}
			});
		});
	...
	// 翻页排序时带上查询条件
		...
		/** 地市下拉框定义 */
		var citiesStore = new Ext.data.SimpleStore( {
			fields : [ 'value', 'text' ],
			data : [ [ '5700000000', '衢州' ], [ '5710000000', '杭州' ],
					[ '5720000000', '湖州' ], [ '5730000000', '嘉兴' ],
					[ '5740000000', '宁波' ], [ '5750000000', '绍兴' ],
					[ '5760000000', '台州' ], [ '5770000000', '温州' ],
					[ '5780000000', '丽水' ], [ '5790000000', '金华' ],
					[ '5800000000', '舟山' ] ]
		});
		/** id,name 不能一样 */
		var citiesCombo = new Ext.form.ComboBox( {
			name : 'grp_city',
			id : 'grp_city1',
			hiddenName : 'grp_city',
			store : citiesStore,
		...
		var citiesCombo_E = new Ext.form.ComboBox( {
			name : 'grp_city',
			hiddenName : 'grp_city',
			id:'editgrp_city',
			store : citiesStore,
		...
		function addInit() {
			addUserWindow
					.setTitle('新增地市管理员<span style="color:Red">(*为必填项)</span>');
			...
			Ext.getCmp('usertype').setValue('8');
			addUserWindow.show();
		}
		...
		function saveUserItem(flag) {
			...
			var msg;
			if (flag == "save") {
				msg = '<span style="color:red">确认保存所添加的地级管理员信息吗?</span>'
			}
			if (flag == "send") {
				msg = '<span style="color:red">确认保存所添加的地级管理员信息并且发送短信给该地级管理员吗?</span>'
			}
		...
		function editInit() {
			...
			editUserWindow.show();
			editUserWindow
					.setTitle('修改地市管理员<span style="color:Red">(*为必填项)</span>');
			...
			Ext.getCmp('editgrp_city').setDisabled(true);
		}
		...
		function validateAcc() {
			...
			if (Ext.getCmp('grp_city1').getValue() == null
					|| Ext.getCmp('grp_city1').getValue() == '') {
				Ext.Msg.alert('提示', '请选择所属地市!');
				return;
			}
			validateAccStore.load( {
				params : {
					account : Ext.getCmp('account').getValue(),
					grp_city : Ext.getCmp('grp_city1').getValue()
				}
			});
		}
		   ...
		   /**
			...
			function sendPwd() {
				...
				Ext.Msg.confirm('请确认',
								'<span style="color:red"><b>提示:</b>确认给该地市管理员发送密码信息吗?</span><br>',
								function(btn, text) {
									if (btn == 'yes') {
										if (runMode == '0') {
