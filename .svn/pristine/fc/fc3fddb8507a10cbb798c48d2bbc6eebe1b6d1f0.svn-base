            ...
            for (i = 0, len = filters.length; i < len; i++) {
                ...
                if (cm) {
                    ...
                    if (filter){
                        ...
                        // filter type is specified in rfid of preference:
...
Ext.ux.StatusBar = Ext.extend(Ext.Toolbar, {
     ...
     * rendered, in added rfid, to the opposite side.  The status element is greedy, so it will automatically
...
Ext.ux.tree.TreeGrid = Ext.extend(Ext.tree.TreePanel, {
        ...
        if(!this.internalTpl){                                
            this.internalTpl = new Ext.XTemplate(
                '<div class="x-grid3-header">',
                    ...
                    '</div></div>',
                '</div>',
                '<div class="x-treegrid-root-node"  style="overflow:'+(this.inlineScroll?'auto':'hidden')+';">',
    ...
    onResize : function(w, h) {
        ...
        if(Ext.isNumber(w)){
			bd.style.width = this.body.getWidth(true) - 1 + 'px';
			hd.style.width = this.body.getWidth(true) - 1 - 16 + 'px';
