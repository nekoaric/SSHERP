...
Ext.onReady(function() {
	var indepd_cwa_ctrl = 0,deptid,managedeptid;// 初始化-0

	var showCount = 0,userid;
    ...
    addDeptTree.on('click', function (node) {
        ...
        addUserFormPanel.findById('deptid').setValue(node.attributes.id);
...
//                                fieldLabel: '暂住证号',
	...
	function saveUserItem(flag) {
		...
		var msg;
		if (flag == "save") {
			msg = '<span style="color:red">确认保存所添加的企业操作员信息吗?</span>'
		}
		if (flag == "send") {
			msg = '<span style="color:red">确认保存所添加的企业操作员信息并且发送短信给该企业操作员吗?</span>'
		}
	...
	function updateUserItem(flag) {
		...
		var msg;
		if (flag == "save") {
			msg = '<span style="color:red">确认保存所修改的企业操作员信息吗?</span>'
		}
		if (flag == "send") {
			msg = '<span style="color:red">确认保存所修改的企业操作员信息并且发送短信给该企业操作员吗?</span>'
		}
		Ext.Msg.confirm('确认', msg, function(btn, text) {
					if (btn == 'yes') {
						update(flag);
	...
	function shutoractiveSysGrpZG(flag) {
		...
		if (flag == "open") {
			if (Ext.isEmpty(rows)) {
				Ext.Msg.alert('提示', '请先选中您要进行启用的操作员信息!');
				return;
			} else {
		...
		if (flag == "shut") {
			if (Ext.isEmpty(rows)) {
				Ext.Msg.alert('提示', '请先选中您要进行停用的操作员!');
				return;
			} else {
	...
	function sendPwd() {
        ...
        Ext.Msg.confirm('请确认',
            '<span style="color:red"><b>提示:</b>确认给该企业操作员发送密码信息吗?</span><br>',
            function (btn, text) {
                if (btn == 'yes') {
                    showWaitMsg('请等待!');
                    Ext.Ajax.request({
    ...
    var check_addRoot = new Ext.tree.AsyncTreeNode({
        text : '',
        id : '001',
        expanded : true
    });

    var check_addDeptTree = new Ext.tree.TreePanel({
        loader : new Ext.tree.TreeLoader({
            dataUrl : './user.ered?reqCode=deptTreeInit'
        }),
        root : check_addRoot,
    ...
    // 监听下拉树的节点单击事件
    check_addDeptTree.on('click', function(node) {
    ...
    check_comboxWithTree.on('expand', function() {
        // 将UI树挂到treeDiv容器
        check_addDeptTree.render('check_addDeptTreeDiv');
        check_addDeptTree.root.expand(); //只是第一次下拉会加载数据
        //						addDeptTree.root.reload(); // 每次下拉都会加载数据
