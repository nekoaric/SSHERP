...
import com.cnnct.sys.web.tag.vo.DeptVo;
import com.cnnct.sys.web.tag.vo.UserVo;
...
public class ArmSelectUserTreeTag extends TagSupport {
	...
	public int doStartTag() throws JspException{
		...
		List deptList = g4Dao.queryForList("queryDeptsForRoleGrant", deptDto);
		...
		for(int i = 0; i < deptList.size(); i++){
			...
			List tempList = g4Dao.queryForList("queryUsersForRoleGrant", userDto);
			userList.addAll(tempList);
		}
		Dto grantDto = new BaseDto();
		grantDto.put("roleid", request.getParameter("roleid"));
		List grantList = g4Dao.queryForList("queryGrantedUsersByRoleId", grantDto);
