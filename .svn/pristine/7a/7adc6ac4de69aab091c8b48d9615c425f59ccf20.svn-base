...
/**
 * 生成通知单150324版本数据获取 由excel解析到的数据，提取出来保存为业务数据的格式
 * 
 * @author zhouww
 * @since 2015-03-24
 */
@SuppressWarnings("unchecked")
public class ProdOrd2EPMode150324 {

    public ProdOrd2EPMode150324() {
    };
     ...
     */
    public static Dto extractData(List<List<String>> list, Dto inDto)
            throws ApplicationException {
        ...
        try {
            ...
            check_prod_date = ProdOrd2ExcelProcess.parseDateFormat(check_prod_date);
...
//            String allow_loss_per = list.get(6).get(7).trim();// 允许损耗
            ...
            String ksfl = list.get(13).get(3).trim(); // 款式分类
            String mlfl = list.get(13).get(1).trim(); // 面料分类
            String sxff = list.get(13).get(6).trim(); // 水洗方法
            String sxyxsh = list.get(12).get(2).trim(); // 水洗允许损耗
            String fzyxsh = list.get(12).get(6).trim(); // 缝制允许损耗
            ...
            sew_start_date = ProdOrd2ExcelProcess.parseDateFormat(sew_start_date);

            String sew_delivery_date = list.get(4).get(14).trim();// 缝制交货日期
            sew_delivery_date = ProdOrd2ExcelProcess.parseDateFormat(sew_delivery_date);

            String bach_delivery_date = list.get(5).get(14).trim();// 水洗交货日期
            bach_delivery_date = ProdOrd2ExcelProcess.parseDateFormat(bach_delivery_date);

            String pack_delivery_date = list.get(6).get(14).trim();// 后整交货日期
            pack_delivery_date = ProdOrd2ExcelProcess.parseDateFormat(pack_delivery_date);
            ...
            plan_check_date = ProdOrd2ExcelProcess.parseDateFormat(plan_check_date);

            String purchase_check_date = list.get(9).get(14).trim();// 采购审批日期
            purchase_check_date = ProdOrd2ExcelProcess.parseDateFormat(purchase_check_date);

            String tech_check_date = list.get(10).get(14).trim();// 技术审批日期
            tech_check_date = ProdOrd2ExcelProcess.parseDateFormat(tech_check_date);

            String trade_check_date = list.get(11).get(14).trim();// 贸易审批日期
            trade_check_date = ProdOrd2ExcelProcess.parseDateFormat(trade_check_date);
...
//            outDto.put("allow_loss_per", allow_loss_per);
            ...
            outDto.put("ksfl", ksfl); // 款式分类
            outDto.put("mlfl", mlfl); // 面料分类
            outDto.put("sxff", sxff); // 水洗方法
            outDto.put("sxyxsh", sxyxsh); // 水洗允许损耗
            outDto.put("fzyxsh", fzyxsh); // 缝制允许损耗
            outDto.put("version", version); // 版本信息
            ...
            // 4.1获取订单数量信息
            List<String> order_list = list.get(15);// 订单数量的头信息
            int beginNumInfo = 16; // 开始解析数量的行
            // 4.2获取腰围信息(头部列信息)
            String column = "";
            ...
            for (int i = beginNumInfo; !isEndOrdNum; i++) {
                List<String> l = list.get(i);
                ...
                if (!"小计".equals(l.get(0))) {
                    ...
                    productList = ProdOrd2ExcelProcess.parseOneRow(beanDto, l, productList,
                            order_list, "ord_num");
                } else if ("小计".equals(l.get(0))) {
                    order_num_cal = l.get(19); // 获得小计统计信息
                    isEndOrdNum = true;
                    subTotalDto = ProdOrd2ExcelProcess.parseSubTotalRow(l, order_list);
                }
            }
            ...
            for (int i = ord_index + 2; !isEndInsNum; i++) {
                ...
                if (!"小计".equals(l.get(0))) {
                    ...
                    // 设置结束条件
                    isEndInsNum = true;
                    totalDto = ProdOrd2ExcelProcess.parseSubTotalRow(l, order_list);
                }
            }
