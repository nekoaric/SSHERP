...
Ext.calendar.CalendarPicker = Ext.extend(Ext.form.ComboBox, {
    ...
    listeners : {
        'select': function(combo,rec){
            ...
            combo.changeWin(combo.getValue());
        }
    },
    // 不同的操作类型采用不用的界面显示
    changeWin :function(cid){
        var aw = Ext.getCmp('arrangewindow');
        M = Ext.calendar.EventMappings;
        var title = Ext.getCmp(M.Title.mapping);
        var poC = Ext.getCmp(M.Order.mapping);
        var amountC = Ext.getCmp(M.Amount.mapping);
        var realAmount = Ext.getCmp(M.RealAmount.mapping);
        var startDate = Ext.getCmp('date-range-start-date');
        var endDate = Ext.getCmp('date-range-end-date');
        var styleno = Ext.getCmp('styleno')
        var queryBut = Ext.getCmp('queryOrderButton');  // 查询订单按钮
        var insNum = Ext.getCmp('insnum');  // 指令数
        
        var delBtn = Ext.getCmp('delete-btn');
        
        //TODO
        var startRemark = Ext.getCmp('startDateRemark'); // 日期的注释 业务员排数使用
        
        var materielDate = Ext.getCmp('materieldate');  // 面料进仓时间
        
        // 设置全显示状态
        title.getEl().up('.x-form-item').setDisplayed(true);
        poC.getEl().up('.x-form-item').setDisplayed(true);
        amountC.getEl().up('.x-form-item').setDisplayed(true);
        startDate.show();
        endDate.show();
        styleno.getEl().up('.x-form-item').setDisplayed(true);
        // 设置默认隐藏
        realAmount.getEl().up('.x-form-item').setDisplayed(false);
        queryBut.hide();
        insNum.getEl().up('.x-form-item').setDisplayed(false);
        materielDate.getEl().up('.x-form-item').setDisplayed(false)
        startRemark.hide();
        ...
        endDate.enable();
        styleno.enable();
        insNum.disable();    // 指令数.不可编辑
        
        // 设置默认的标签信息
        amountC.getEl().up('.x-form-item').select('.x-form-item-label').update('生产数量:');  //修改标签
        
        switch(+cid){
            case 1 : 
                styleno.getEl().up('.x-form-item').setDisplayed(false);
                poC.getEl().up('.x-form-item').setDisplayed(false);
                amountC.getEl().up('.x-form-item').select('.x-form-item-label').update('前道目标产量:');  //修改标签
                break;
            case 2 : 
                poC.getEl().up('.x-form-item').setDisplayed(false);
                delBtn.hide();
                break;
            case 3 :
                insNum.getEl().up('.x-form-item').setDisplayed(true);
                endDate.hide();
                if(isGrpUser && +isGrpUser===1){
                    delBtn.hide();
                }
                ...
                queryBut.show();
                startRemark.show();
                materielDate.getEl().up('.x-form-item').setDisplayed(true);
                
                // 填充显示信息的内容
                if(aw.isAdd){ // 如果是新增则初始化 初始化日期
                    materielDate.setValue(startDate.getValue());
                }
                break;
            case 4 : 
                title.disable();
                ...
                endDate.hide();
                amountC.getEl().up('.x-form-item').setDisplayed(false);
                delBtn.hide();
                realAmount.getEl().up('.x-form-item').setDisplayed(true);
                break;
            case 5 :
                insNum.getEl().up('.x-form-item').setDisplayed(true);
            ...
            case 8 :
                ...
                endDate.hide();
                // 填充显示信息的内容
                if(aw.isAdd){ // 如果是新增则初始化title, 初始化日期
                    materielDate.setValue(startDate.getValue());
                    title.setValue('款式--(数量)/(指令数)--订单号');
                }
                break;
        }
        if(!aw.isAdd){
            // 如果是修改的话那么需要对组件进一步做限制 
            switch(+cid){
                case 1 : 
                    startDate.disable();
                    endDate.disable();
...
Ext.calendar.CalendarView = Ext.extend(Ext.BoxComponent, {
    ...
    onEventDrop: function(rec, dt) {
        if (Ext.calendar.Date.compare(rec.data[Ext.calendar.EventMappings.StartDate.name], dt) === 0 
            || !allowCalendarView() || false ) {    // 所有人员不能够拖动
            //add 2015-06-18  zhouww
            // no changes  超级管理员是不能够拖动
            return;
        }
    ...
    setStartDate: function(start, refresh) {
        ...
        params = params || {};
        params.startDate = this.viewStart.format('Y-m-d');
        params.endDate = this.viewEnd.format('Y-m-d');
            // 添加到基本查询参数对象中
...
Ext.calendar.MonthView = Ext.extend(Ext.calendar.CalendarView, {
    ...
    onDetailViewUpdated: function(view, dt, numEvents) {
        ...
        bodyH = frameH + (numEvents * evtH) + 23,
...
Ext.calendar.EventEditWindow = function(config) {
    ...
    // 查询订单的回调函数
    function queryOrderBackFn(res){
        // 处理选择一条数据的情况
        if(res.length === 1){
            console.log(res);
            var record = res[0];
            // 获取数据后赋值数量信息
            var styleno = record.get('style_no');
            var order = record.get('order_id');
            var insNum = record.get('ins_num');
            
            var title = styleno + '-- (数量)/('+insNum+') --' + order;
            
            // 为formPanel赋值
            Ext.getCmp('title').setValue(title);
            Ext.getCmp('styleno').setValue(styleno);
            Ext.getCmp('order').setValue(order);
            Ext.getCmp('insnum').setValue(insNum);
        }else {
            Ext.Msg.alert('title','选择一个订单!');
            return false;
        }
        
    }
    // 查询订单的点击处理事件 
    function queryButtonClickFn(){
            var qwc = new QueryWindowConstruct()
            qwc.addListener('997',queryOrderBackFn);
            qwc.showQueryWindow();
            console.log('queryButtonClickFn');
    }
    ...
    var genItems = [{
        ...
        id : 'styleno',
        name : Ext.calendar.EventMappings.StyleNo.name,
        fieldLabel : '款式',
        xtype : 'textfield',    
        anchor : '100%'
    },{
        id:"order",
        name : Ext.calendar.EventMappings.Order.name,
        fieldLabel:'PO#(订单号)',
        xtype:'textfield',
        anchor:'100%'
    },{
        id : 'insnum',
        name : Ext.calendar.EventMappings.InsNum.name,
        fieldLabel : '指令数',
        xtype : "numberfield",
        allowDecimals : false,
        allowNegative : false,
        ...
        {
            id : 'amount',
            name : Ext.calendar.EventMappings.Amount.name,
            fieldLabel: "生产数量",
            xtype : "numberfield",
            allowDecimals : false,
            allowNegative : false,
        ...
        {
         id : 'realamount',
            name : Ext.calendar.EventMappings.RealAmount.name,
            fieldLabel: "生产数量",
            xtype : "numberfield",
            allowDecimals : false,
            allowNegative : false,
        ...
        },
        {
            // 面辅料到仓日期
            id:"materieldate",
            name : Ext.calendar.EventMappings.MaterielDate.name,
            fieldLabel: '面辅料进仓日期',
            xtype : 'datefield',
            value : new Date(),
            format : 'n/j/Y'
        },
        {
            ...
            text : '注:您所选日期必须在辅料到仓之后'
            // 业务员开始日期注释
        }];

    for(var idx=0;idx<genItems.length;idx++){
        formPanelCfg.items.push(genItems[idx]);
    }

    Ext.calendar.EventEditWindow.superclass.constructor.call(this, Ext.apply({
        titleTextAdd: '新增一个排单计划',
        titleTextEdit: '修改这个排单计划',
        width: 600,
        id : 'arrangewindow',
        autocreate: true,
        border: true,
...
//TODO
        ...
        if (o.data) {
            ...
            rec.data[M.IsAllDay.name] = true;   //2015-06-26 zhouww 所有的选择都是全选，不采用下面的判断了 
//            !!o[M.IsAllDay.name] || start.getDate() != end.clone().add(Date.MILLI, 1).getDate();
    ...
    createFormRecord : function(){
        ...
        M = Ext.calendar.EventMappings;
        var formRecord = this.activeRecord.copy();
        f.updateRecord(formRecord);
        
        var orderNo = Ext.getCmp(M.Order.mapping).getValue();
        var amount = Ext.getCmp(M.Amount.mapping).getValue();
        var styleNo = Ext.getCmp(M.StyleNo.mapping).getValue();
        var title = Ext.getCmp(M.Title.mapping).getValue();
        var ins_num = Ext.getCmp(M.InsNum.mapping).getValue();
        ins_num = ins_num ? ins_num : '-';
        // 如果是空的title那么组合'款式--(数量)/(指令数)--订单'格式的数据
        if(Ext.isEmpty(title)){
            title = styleNo + '--('+amount+')/('+ins_num+')--' + orderNo;
            formRecord.set(M.Title.name,title);
            Ext.getCmp(M.Title.mapping).setValue(title);
        }else if(title.indexOf('数量')>=0 || title.indexOf('款式') >= 0
                || title.indexOf('指令数') >= 0 || title.indexOf('订单号') >=0){ 
            // 如果有'数量'字符串的情况，那么替换这个字符串
            title = title.replace('数量',amount);
            // 如果有'款式'那么替换这个字符串
            title = title.replace('款式',styleNo);
            title = title.replace('指令数',ins_num);
            title = title.replace('订单号',orderNo);
            formRecord.set(M.Title.name,title);
            Ext.getCmp(M.Title.mapping).setValue(title);
        }
        
        formRecord.set(M.StartDate.name, dates[0]);
        formRecord.set(M.EndDate.name, dates[1]);
        formRecord.set(M.IsAllDay.name, dates[2]);
        // 添加订单号
        formRecord.set(M.Order.name,orderNo);
        // 添加数量
        formRecord.set(M.Amount.name,amount);
        formRecord.set(M.CalendarId.name, this.formPanel.form.findField('calendar').getValue());
        return formRecord;
    },
    ...
    updateRecord: function() {
        ...
        // 如果是空的title那么组合'款式--数量--订单'格式的数据
        if(Ext.isEmpty(title)){
            title = styleNo + '--('+amount+')--' + orderNo;
            this.activeRecord.set(M.Title.mapping,title);
            Ext.getCmp(M.Title.mapping).setValue(title);
        }else if(title.indexOf('数量')>=0){ 
            // 如果有'数量'字符串的情况，那么替换这个字符串
            title = title.replace('数量',amount);
            this.activeRecord.set(M.Title.mapping,title);
            Ext.getCmp(M.Title.mapping).setValue(title);
        ...
        // 添加订单号
        this.activeRecord.set(M.Order.name,Ext.getCmp(M.Order.mapping).getValue());
        // 添加数量
        this.activeRecord.set(M.Amount.name,Ext.getCmp(M.Amount.mapping).getValue());
    ...
    onSave: function() {
        ...
        // 创建临时record
        var tR = this.createFormRecord();
//        this.updateRecord();

        if (!this.activeRecord.dirty && false) {
            this.onCancel();
            return;
        }

//        var isValide = this.fireEvent(this.isAdd ? 'eventadd': 'eventupdate', this, this.activeRecord);
        var isValide = this.fireEvent(this.isAdd ? 'eventadd': 'eventupdate', this, tR);
        //TODO 如果在test-app业务逻辑中添加/修改内容后重新加载数据 ，就不需要更新当前store
        //如果 在test-app业务逻辑中添加/修改内容后没有重新加载数据，那么就需要使用下面的方式来更新界面
//        if(isValide){
//            // 只有成功后才更新record
//            this.updateRecord();
//        }
