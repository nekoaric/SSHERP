...
Ext.onReady(function () {
	...
	var validateAccStore = new Ext.data.Store({
		proxy: new Ext.data.HttpProxy({
			url: './manageUser.ered?reqCode=validateAcc' // 后台请求地址
		}),
		reader: new Ext.data.JsonReader({}, [
			{ // 定义后台返回数据格式
				name: 'cnt' // 数量
	...
	var store = new Ext.data.Store({
		proxy: new Ext.data.HttpProxy({
			url: './manageUser.ered?reqCode=queryUsersForManage'
		}),
		reader: new Ext.data.JsonReader({
			...
			'-',
			{
				...
				handler: function () {
					...
					Ext.Msg.confirm('确认', "确认删除该用户角色权限信息吗?", function (btn, text) {
						if (btn == 'yes') {
							Ext.Ajax.request({
								url: './manageUser.ered?reqCode=delSysUserRoleMap',
	...
	var addDeptTree = new Ext.tree.TreePanel({
		loader: new Ext.tree.TreeLoader({
			baseAttrs: {},
			dataUrl: './manageUser.ered?reqCode=departmentTreeInit'
		}),
		root: addRoot,
		autoScroll: true,
		animate: false,
		...
		Ext.Msg.confirm('确认', msg, function (btn, text) {
			if (btn == 'yes') {
				addUserFormPanel.getForm().submit({
					url: './manageUser.ered?reqCode=saveUserItem',
					waitTitle: '提示',
					method: 'POST',
					waitMsg: '正在处理数据,请稍候...',
	...
	function editInit() {
		...
		Ext.getCmp('roleid_old').setValue(record.get('role_id'));
		...
		addUserFormPanel.form.load({
			...
			url: 'manageUser.ered?reqCode=loadUserInfo',
			success: function (form, action) {
			},
			failure: function (form, action) {
	...
	function updateUserItem() {
		...
		Ext.Msg.confirm('确认', msg, function (btn, text) {
			if (btn == 'yes') {
				addUserFormPanel.getForm().submit({
					url: './manageUser.ered?reqCode=updateUserItem',
					waitTitle: '提示',
					method: 'POST',
					waitMsg: '正在处理数据,请稍候...',
					success: function (form, action) {
