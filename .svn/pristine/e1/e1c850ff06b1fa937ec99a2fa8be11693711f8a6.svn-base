...
public class BaseAction extends DispatchAction {
	...
	protected SessionContainer getSessionContainer(HttpServletRequest request) {
		...
		if (sessionContainer == null) {
			...
			//保存客户端的一些信息
			Dto clientDto = new BaseDto();
			clientDto.put("useragent", request.getHeader("user-agent"));
			clientDto.put("remoteHost", request.getRemoteHost());
			HttpSession session = request.getSession(true);
			session.setAttribute("clientInfo", clientDto);
	...
	/**
	 * 获取当前登录用户的基本信息（单位代码，单位编号，用户编号，姓名，类型）
	 * @param request
	 * @return
	 */
	public Dto getDto4UserInfo(HttpServletRequest request){
	    Dto dto = new BaseDto();
	    
	    UserInfoVo userInfo = getSessionContainer(request).getUserInfo();
	    dto.put("grp_id", userInfo.getGrpId()); // 用户所属单位代码
	    dto.put("dept_id", userInfo.getDeptid()); // 用户所属单位编号
	    dto.put("opr_id", userInfo.getAccount()); // 用户编号
	    dto.put("opr_name", userInfo.getUsername()); // 用户姓名
	    dto.put("user_type", userInfo.getUsertype()); // 用户类型
