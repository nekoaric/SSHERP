...
public class RolePartAction extends BaseAction {
	...
	public ActionForward queryParts(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		List list = g4Reader.queryForPage("queryParts", dto);
		Integer countInteger = (Integer) g4Reader.queryForObject("queryPartsForPageCount", dto);
		for (int i = 0; i < list.size(); i++) {
			Dto partDto = (BaseDto)list.get(i);
			dto.put("partid", partDto.getAsString("partid"));
			Dto outDto = (BaseDto)g4Reader.queryForObject("queryPart4RoleGrant", dto);
			if (G4Utils.isEmpty(outDto)) {
				partDto.put("partauthtype","2");
			}else {
