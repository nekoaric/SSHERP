...
Ext.onReady(function () {
	...
	var detail_store = new Ext.data.Store({
		proxy: new Ext.data.HttpProxy({
			url: 'deptSche.ered?reqCode=getDeptDaySche'
		}),
		reader: new Ext.data.JsonReader({
			totalProperty: 'TOTALCOUNT',
			root: 'ROOT'
		}, ['dept_name','tr_date', 'nature','day_num','complete_num', 'total_num', 'article', 'brand', 'ord_seq_no', 'order_date',
			'cust_id','dept_id'
		])
	});
	// 翻页排序时带上查询条件
	...
	var detail_grid = new Ext.grid.GridPanel({
		...
		tbar: [
			...
			handler: function () {
				...
				handler: function () {
					exportExcel('./deptSche.ered?reqCode=exportDeptDaySche&flag=cur');
				}
			},{
				text: '导出全部记录',
				id: 'import_button',
				iconCls: 'page_excelIcon',
				handler: function () {
					exportExcel('./deptSche.ered?reqCode=exportDeptDaySche&flag=all');
				}
			}
		],
		bbar: detail_bbar
...
var empTab = new Ext.TabPanel({
	...
	function updateChartData() {
		Ext.Ajax.request({
			url: './deptSche.ered?reqCode=getDeptDayScheView',
			params: {
