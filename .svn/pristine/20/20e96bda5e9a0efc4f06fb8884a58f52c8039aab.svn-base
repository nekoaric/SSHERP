...
//        System.setProperty("jxl.nogc", Boolean.toString(true));
        WorkbookSettings wkbkSettings = new WorkbookSettings();
        wkbkSettings.setInitialFileSize(102400);
        wkbkSettings.setArrayGrowSize(102400);
        wkbkSettings.setGCDisabled(true);
		WritableSheet wSheet = null;
		ByteArrayOutputStream bos = new ByteArrayOutputStream();
		try {
			Workbook wb = Workbook.getWorkbook(new File(getExcelTemplate().getTemplatePath()),wkbkSettings);
	...
	private void fillFields(WritableSheet wSheet) throws Exception {
        ...
        if(fieldList.size()>65536){
            throw new Exception("不支持超过65536行的数据!");
        }
		for (int j = 0; j < fieldList.size(); j++) {
			...
			NumberFormat format$ = new NumberFormat("###,##0.00");//设置金额格式    
			//xtj 8-19 增加数值格式
			NumberFormat format = new NumberFormat("#####0"); 
				...
				try {
					if (type.equalsIgnoreCase(GlobalConstants.ExcelTPL_DataType_Number)) {
						Number number = new Number(cell.getColumn(), cell.getRow() + j, dataDto.getAsBigDecimal(key)
								.doubleValue());
						number.setCellFormat(cellFormat);//设置数值格式 
						wSheet.addCell(number);
					} else {
	...
	private void fillVariables(WritableSheet wSheet, int row) {
		...
		for (int i = 0; i < variables.size(); i++) {
			...
			try {
				if (type.equalsIgnoreCase(GlobalConstants.ExcelTPL_DataType_Number)) {
					//8-19xtj
					Number number = new Number(cell.getColumn(), row, parameterDto.getAsInteger(key).intValue());
					number.setCellFormat(cell.getCellFormat());
					wSheet.addCell(number);
				} else {
