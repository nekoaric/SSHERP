...
Ext.onReady(function() {
	...
	// 翻页排序时带上查询条件
		...
		var grid = new Ext.grid.GridPanel(
				{
					...
					tbar : [
							...
							//						handler : function() {
							//					    exportExcel('./user.ered?reqCode=downUserExcel');
							//						}
							//					   },
							'-',
							{
		...
		function addInit() {
			...
			Ext.getCmp('usertype').setValue('6');
			sexCombo.setValue('1');
		...
		function saveUserItem(flag) {
			...
			var msg;
			if (flag == "save") {
				msg = '<span style="color:red">确认保存所添加的地市客户经理信息吗?</span>'
			}
			if (flag == "send") {
				msg = '<span style="color:red">确认保存所添加的地市客户经理信息并且发送短信给该地市客户经理吗?</span>'
			}
		...
		/**
			...
			function sendPwd() {
				...
				var record = grid.getSelectionModel().getSelected();
				if (Ext.isEmpty(rows)) {
					Ext.Msg.alert('提示', '请先选中您要进行发送密码的客户经理信息!');
					return;
				}
				var strChecked = jsArray2JsString(rows, 'userid');
				Ext.Msg.confirm('请确认',
