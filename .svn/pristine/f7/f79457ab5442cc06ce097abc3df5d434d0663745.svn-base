...
public class GeneralStatement extends BaseStatement {
    ...
    protected void executeQueryWithCallback(RequestScope request, Connection conn, Object parameterObject, Object resultObject, RowHandler rowHandler, int skipResults, int maxResults)
        ...
        try {
            ...
            //modify by meif :获取一些手工添加的参数
            String currentLoginedUserId = "";
            String queryForPageCountFlag ="";
            if(parameterObject instanceof Dto){
                Dto dto = (Dto)parameterObject;

                currentLoginedUserId=dto.getAsString("currentLoginedUserId");
                queryForPageCountFlag=dto.getAsString("queryForPageCountFlag");
            }
            ...
            //modify by meif :往执行查询的语句中添加参数
            if(!"".equals(currentLoginedUserId)){
                //当前登入用户string由"currentLoginedUserId,"+userid 组成
                currentLoginedUserId = "currentLoginedUserId"+","+currentLoginedUserId;
                parameters[parameters.length-1]=currentLoginedUserId;
            }
            if(!"".equals(queryForPageCountFlag)){
                parameters[parameters.length-2]=queryForPageCountFlag;
            }
