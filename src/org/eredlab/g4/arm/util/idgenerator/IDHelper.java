...
/**
 * ID生成器 静态类解决多线程并发访问生成ID的问题 此类第一次实例化会执行所有的static代码块，如果想按需加载这些ID生成器，则应该一个ID写一个静态类就可以
 * 
 * @author XiongChun
 * @since 2010-09-16s
 */
public class IDHelper {
     ...
     * REGNO 员工社保  就业失业登记证号
     */
    private static DefaultIDGenerator defaultIDGenerator_regno=null;
     ...
     * DEVDOWNCTRLSERNO
     */
    private static DefaultIDGenerator defaultIDGenerator_DevDownCtrlSerNo = null;
    ...
    static {
        IdGenerator idGenerator_devDownCtrlSerNo = new IdGenerator();
        idGenerator_devDownCtrlSerNo.setFieldname("DevDownCtrlSerNo");
        defaultIDGenerator_DevDownCtrlSerNo = idGenerator_devDownCtrlSerNo.getDefaultIDGenerator();
    }
    
    
    static {
    	IdGenerator idGenerator_Regno=new IdGenerator();
    	idGenerator_Regno.setFieldname("REGNO");
    	defaultIDGenerator_regno=idGenerator_Regno.getDefaultIDGenerator();
    }
     ...
     * @return
     */
    public static String getDevDownCtrlSerNo() {
        return defaultIDGenerator_DevDownCtrlSerNo.create();
    }
    
    
    /**
     * 返回REGNO 社保信息 员工社保就业失业登记证号
     * @return
     */
    public static String getDefaultIDGeneratorRegno() {
		return defaultIDGenerator_regno.create();
	}
}
