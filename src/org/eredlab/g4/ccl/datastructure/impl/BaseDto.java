...
public class BaseDto extends HashMap implements Dto, Serializable {
    ...
    public BaseDto( HttpServletRequest request){
        //封装dto时,初始化当前登入的用户信息
        UserInfoVo user = WebUtils.getSessionContainer(request).getUserInfo();
        String currentLoginedUserId = DataPermUtil.getDataPermInfoByUserInfo(user);
        put("currentLoginedUserId",currentLoginedUserId);
    }
