...
var belong_grp_id;
var areaInfo = {};    //地区信息
var oldAreaInfo={}; // 这个记录查询订单时的地区信息，用于判断areaInfo和ordAreaInfo不一样的情况
...
westCenterTreePanel.on('click', function(node) {
			...
			belong_grp_id = node.attributes.belong_grp_id;   // 保存部门信息
            // 判断是否在同一个地区下 用来修改查询订单的信息
            var nAreaInfo = getAreaInfo(node);
           // 赋值新地区信息
            areaInfo = nAreaInfo;
            if(oldAreaInfo.area_id != areaInfo.area_id){  // 点击的地区和新地区不一致
                queryDetailOrder();
            }
            ...
            // 选择地区后隐藏选择框
            areaWindow.hide(); 
            document.getElementById('showAreaInfo').innerHTML = areaInfo.area_text;
		});

/**
 * 查询订单详细信息
 */        
function queryDetailOrder(){
    var queryWindow = new QueryWindowConstruct();
    queryWindow.addListener('23',queryDetailOrderListener);
//    queryWindow.showQueryWindow({
//        area_no : areaInfo.area_text
//    });
    queryWindow.showQueryWindow({});
}
/**
...
function getAreaInfo(node){
    var tAreaId = node.attributes.id;
    var tAreaText = node.attributes.text;
    var length = tAreaId.length;
    if(length <= 13){
        return {
                area_id : tAreaId,
                area_text : tAreaText
        ...
        return getAreaInfo(node.parentNode);
    }
}
 ...
 * @param {} record
 */
function queryDetailOrderListener(record){
    oldAreaInfo = areaInfo;
    var order_id = record[0].get('order_id');
    loadCenterPanelInfo(order_id);
}
...
var centerPanel = new Ext.form.FormPanel({
	...
	items : [{
        ...
        items : [{
            ...
            columnWidth : .2,
            html : '<div>当前选择的地区是:<span style="color:red" id="showAreaInfo"></span></div>'
        }]
    },{
				layout : 'column',
				border : false,
...
var addWindow = new Ext.Window({
...

			{
				...
				iconCls : 'acceptIcon',
				handler : function() {
                    if(areaInfo.area_id != oldAreaInfo.area_id){
                        alert('改变地区后需要重新查询订单，如果没有订单，请联系负责人');
                        return ;
                    }
					// 数据完整性判断
