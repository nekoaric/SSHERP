		...
		.onReady(function() {
			...
			var cm = new Ext.grid.ColumnModel( [
					...
					},
					{
						...
						width : 60,
						renderer : function(value) {
							return nationStore.getAt(nationStore.findExact(
...
//									},
			...
			function addInit() {
				...
				addUserWindow.show();
				addUserWindow
						.setTitle('新增省级操作员<span style="color:Red">(*为必填项)</span>');
				// 为下拉框赋初始值
				lockedCombo.setValue('0');
				Ext.getCmp('usertype').setValue('0');
				sexCombo.setValue('1');
			...
			function saveUserItem(flag) {
				...
				var msg;
				if (flag == "save") {
					msg = '<span style="color:red">确认保存所添加的省级操作员信息吗?</span>'
				}
				if (flag == "send") {
					msg = '<span style="color:red">确认保存所添加的省级操作员信息并且发送短信给该省级操作员吗?</span>'
				}
			...
			function sendPwd() {
				...
				var strChecked = jsArray2JsString(rows, 'userid');
				Ext.Msg.confirm('请确认',
								'<span style="color:red"><b>提示:</b>确认给该省级操作员发送密码信息吗?</span><br>',
								function(btn, text) {
									if (btn == 'yes') {
										if (runMode == '0') {
